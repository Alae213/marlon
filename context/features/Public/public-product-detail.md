...
The PDP at `app/[slug]/product/[productId]/page.tsx` is a live browse experience for anonymous shoppers. Current: it loads the store, navbar snapshot, delivery pricing, and the store's product list on the client, finds the requested product in memory, and supports quantity changes, variant selection, add-to-cart, and a direct "Buy Now" dialog. Partial: the browse flow is real, but public order submission from this page is not truly live because it calls the owner-only `api.orders.createOrder` mutation.
...
3. If the shopper uses `Buy Now`, the dialog collects customer details, computes delivery and totals client-side, and attempts `useMutation(api.orders.createOrder)`.
...
- Partial: the success state on this page can only be reached when the owner-scoped order mutation succeeds. That makes anonymous checkout from the PDP non-live in the current runtime.
- Partial: the PDP generates one order number for the mutation call and a second order number for the success screen, so the displayed confirmation number may not match the stored one if submission ever succeeds through this path.
...
| Direct order flow | Partial: UI exists, but anonymous submission path is not truly live | Anonymous checkout succeeds through a real public-safe server path |
...
- Partial: the page attempts to submit customer PII directly through `api.orders.createOrder`, but `convex/orders.ts` rejects anonymous users and only allows store owners. That fail-closed behavior is safer than an open public write, but it means the checkout path is not live.
- Partial: phone validation is currently client-side (`lib/phone-validation.ts`) before the mutation attempt; the repo does not yet show a dedicated anonymous submission path with public abuse controls or durable rate limiting.
...
- Should the future anonymous order path live as a dedicated public Convex mutation, a hardened Next route, or another server entry point? The repo currently has both an owner-only mutation path and an unused mock route.
